...
enum custom_keycodes {
    /*
     * Start the custom keycode range at QK_KB_0 so that Vial can map
     * these codes to friendly names via the customKeycodes section in
     * vial.json.  See https://get.vial.today/docs/custom_keycode.html
     * for details.  The order here must match the order in
     * vial.json.
     */
    RUEN_TOGGLE = QK_KB_0,
    RUEN_EN,
    ...
    if (record->event.pressed) {
        switch (keycode) {
            ...
            case RUEN_SYNC:
                ...
                return false;
            case RUEN_MOD:
                /* Tap toggles the layout, hold restores on release. */
                ruen_toggle_layout();
